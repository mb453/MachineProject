...
//const std::wstring imagesDir = L"/resources/images";
 ...
 */
ViewEdit::ViewEdit(wxFrame* parent, std::wstring imagesDir) :wxScrolledCanvas(parent, wxID_ANY)
{
    ...
    parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewEdit::OnEditMachineDrawable, this, XRCID("ChangeMachines"));
}
 ...
 * @param event The event we update
 */
void ViewEdit::OnEditMachineDrawable(wxCommandEvent& event)
{

    for(auto item : GetPicture()->GetMachineDrawable())
    {
        mMDraw = item;
        mMDraw->ShowDialogBox(this);
