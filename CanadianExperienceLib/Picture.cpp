...
void Picture::Save(const wxString& filename)
{
    ...
    int sto = GetMachineDrawable()[0]->GetMachNum();

    int sto2 = GetMachineDrawable()[1]->GetMachNum();


    root->AddAttribute(L"mach1num", wxString::Format(wxT("%i"), sto));
    root->AddAttribute(L"mach2num", wxString::Format(wxT("%i"), sto2));
...
void Picture::Load(const wxString& filename)
{
    ...
    mMDraw[0]->SetMachNum(wxAtoi(root->GetAttribute(L"mach1num", L"1")));

    mMDraw[1]->SetMachNum(wxAtoi(root->GetAttribute(L"mach2num", L"2")));
